...
async def _call_openrouter(text: str) -> dict | None:
    """Parse property text via OpenRouter."""
...
async def parse_property_from_text(text: str) -> dict:
    ...
    Priority: OpenRouter → regex fallback.
    """
    try:
        parsed = await _call_openrouter(text)
        logger.info("OpenRouter parsing successful")
        return _normalize(parsed)
    except Exception as exc:
        logger.error(f"OpenRouter parsing failed, using regex fallback: {exc}")
